...
<section id='distant-reading' class='paginated' aria-label='Linguistic features'>
  ...
  :axis-options="{
    ...
    marginLeft: 37,
    marginRight: 20,
    x: { type: 'utc', label: 'mins', ticks: false, line: true },
  ...
  :options="{
    ...
    y: { percent: true, label: null, ticks: false },
    fy: { tickFormat: null },
    color: { type: 'categorical', range: Object.values(scheme), domain: Object.keys(scheme) },
    marks: [
      Plot.axisX({ ticks: 'minute', tickFormat: d => Math.floor(d / 60000) }),
      Plot.areaY(data, { x: 'time', y: 'value', fill: 'type', title: 'type', curve: 'basis', fy: 0 }),
      Plot.lineY(data, { x: 'time', y: 'arousal', curve: 'linear', fy: 1 })
    ]
  }"
  :pointer-options="{
    width: 1136,
    ...
    marginRight: 20,
    marginTop: 20,
    x: { type: 'utc', label: null, ticks: false, domain: [0, data[data.length - 1].time] },
    marks: [
      Plot.ruleX(data, Plot.pointerX({ x: 'time', stroke: isDark ? 'oklch(0.7935 0.10034 276.98)' : 'oklch(0.4668 0.19179 337.977)', ruleStrokeWidth: 2 }))
    ]
  }"
  />
</section>
